      ...
      <div className="flex gap-6 flex-col lg:flex-row">
        ...
        <article className="flex-[1.4] flex flex-col gap-5">
          <button
            ...
            className="block relative w-full h-[560px] overflow-hidden group cursor-pointer text-left"
            ...
            <div className="absolute inset-0 p-7 flex flex-col justify-between text-cream">
              ...
              <div>
                ...
                <div className="flex items-center mt-4 gap-3">
                  <span className="w-12 h-12 bg-lime flex items-center justify-center text-black rounded-full">
                    <PlayIcon />
                  </span>
          ...
          <p className="text-base leading-[1.5] max-w-[520px]">{featured.body}</p>
        </article>
        ...
        <div className="flex-1 flex flex-col gap-4">
          {works.map((w, i) => (
            <button
              ...
              key={w.slug}
              onClick={() => open(w)}
              className={`flex gap-4 group items-center cursor-pointer text-left w-full ${
                i < works.length - 1
            ...
            >
              <div className="relative w-32 h-32 shrink-0 overflow-hidden">
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-black/0 group-hover:bg-black/30 transition-colors">
                  <span className="w-9 h-9 bg-lime rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <PlayIcon small />
                  </span>
                </div>
              </div>
              <div className="flex-1 flex flex-col gap-1.5">
                ...
                <p className="text-[13px] leading-[1.4] line-clamp-2">{w.body}</p>
              </div>
              <span className="w-9 h-9 bg-lime flex items-center justify-center text-black shrink-0 group-hover:bg-[#a8e632] transition-colors rounded-full">
                <PlayIcon small />
              </span>
            </button>
          ))}
...
function VideoModal({ work, onClose }: { work: Work; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4 text-cream">
          <div className="flex flex-col">
            <div className="text-[10px] font-bold tracking-[0.14em] uppercase opacity-70">
              ● {work.category}
            </div>
            <div className="font-display text-[28px] sm:text-[40px] leading-[0.95] tracking-[-0.02em] uppercase mt-1">
              {work.title}
            </div>
          </div>
          <button
            ...
            className="w-12 h-12 bg-lime text-black flex items-center justify-center text-2xl font-bold rounded-full hover:bg-[#a8e632] transition-colors"
          >
            ✕
          </button>
        </div>
        ...
        <p className="mt-4 text-cream/85 text-sm sm:text-base leading-[1.5] max-w-2xl">
          {work.body}
        </p>
      </div>
    </div>
